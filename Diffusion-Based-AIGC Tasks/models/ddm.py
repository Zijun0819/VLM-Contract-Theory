...
class Net(nn.Module):
    ...
    def sample_training(self, x_cond, b, eta=0.):
        skip = self.config.diffusion.num_diffusion_timesteps // 10
...
class DenoisingDiffusion(object):
    def __init__(self, args, config):
        ...
        self.optimizer, self.scheduler = utils.get_optimizer(self.config, self.model.parameters())
    ...
    def load_ddm_ckpt(self, load_path, ema=False):
        ...
        self.model = torch.load(load_path, map_location=self.device)
        # if ema:
    ...
    def train_client(self, client_loader):
        ...
        optimizer, _ = utils.get_optimizer(self.config, local_model.parameters())
    ...
    def sample_validation_patches(self, val_loader, step, formatted_time):
        ...
        with torch.no_grad():
            ...
            for i, (x, y) in enumerate(val_loader):
                ...
                utils.save_image(pred_x, os.path.join(image_folder, str(step), f"{y[0]}.png"))
